function [n_equal_1_amplitude, n_equal_1_normalized, n_equal_1_phase] = ...
  get_n_equal_1_amplitude_using_BP15(shot, timebase);

% Calculate n=1 amplitude and phase for the disruption warning database
% using the four BP15 Bp sensors near the midplane on the outboard vessel
% wall.  The calculation is done by using a least squares fit to an
% expansion in terms of n = 0 & 1 toroidal harmonics.  The BP15 sensors are
...
% We use the signals from 4 toroidally-distributed BP15 sensors, all on
...
bp15_names = {'BP15_BC', 'BP15_DE', 'BP15_GH', 'BP15_JK'};
nsensors = length(bp15_names);
ntimes = length(timebase);
bp15_signals = NaN(nsensors, ntimes);
...
[~,bp15_indices,~] = intersect(bp_nodenames, bp15_names);

bp15_phi = phi(bp15_indices) + 360;
bp15_btor_pickup_coeffs = btor_pickup_coeffs(bp15_indices);
...
for isensor = 1:nsensors;
  [signal, status] = mdsvalue(bp15_names(isensor));  % tesla
...
% Subtract Btor pickup
  ...
  signal = signal - bp15_btor_pickup_coeffs(isensor) * btor;

% Interpolate Bp onto requested timebase 

  bp15_signals(isensor,:) = interp1(magtime, signal, timebase_column);
end;
mdsclose;
...
A(:,2) = transpose(sin(bp15_phi*pi/180));
A(:,3) = transpose(cos(bp15_phi*pi/180));

% Solve the linear least squares problem to get the fitting coefficients
% for all time values

coeffs = A \ bp15_signals;
