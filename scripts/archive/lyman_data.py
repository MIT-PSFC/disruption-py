...
def get_cmod_kin_profs(shot, tmin, tmax, geqdsk=None, pre_shift_TS=False, force_to_zero=False, frac_err=True, num_mc=100,
    ...
    try:
        ...
        p_Te= profiletools.Te(int(shot), include=['ETS'], abscissa='sqrtpsinorm',t_min=tmin,t_max=tmax, remove_zeros=True)
        p_ne= profiletools.ne(int(shot), include=['ETS'], abscissa='sqrtpsinorm',t_min=tmin,t_max=tmax, remove_zeros=True)
    ...
    reg = [4,8] # these will determine which coefficients in the fit are set to zero to prevent overfitting
    ...
    if pre_shift_TS:
        ...
        ne, ne_popt, ne_perr, ne_chisqr = mtanh_fitting.best_osbourne(p_ne.X[idxs_ne,0][mask_ne], p_ne.y[idxs_ne][mask_ne], vals_unc=p_ne.err_y[idxs_ne][mask_ne], x_out=rhop_kp, maxfev=maxfev, reg=reg, edge_chi=edge_chi, ped_width=ped_width, ne=True)
        Te, Te_popt, Te_perr, Te_chisqr = mtanh_fitting.best_osbourne(p_Te.X[idxs_Te,0][mask_Te], p_Te.y[idxs_Te][mask_Te], vals_unc=p_Te.err_y[idxs_Te][mask_Te], x_out=rhop_kp, maxfev=maxfev, reg=reg, edge_chi=edge_chi, ped_width=ped_width, ne=False)
        ...
        ne, ne_popt, ne_perr, ne_chisqr = mtanh_fitting.best_osbourne(p_ne.X[idxs_ne,0][mask_ne], p_ne.y[idxs_ne][mask_ne], vals_unc=p_ne.err_y[idxs_ne][mask_ne], x_out=rhop_kp, maxfev=maxfev, reg=reg, edge_chi=edge_chi, ped_width=ped_width, ne=True)
        Te, Te_popt, Te_perr, Te_chisqr = mtanh_fitting.best_osbourne(p_Te.X[idxs_Te,0][mask_Te], p_Te.y[idxs_Te][mask_Te], vals_unc=p_Te.err_y[idxs_Te][mask_Te], x_out=rhop_kp, maxfev=maxfev, reg=reg, edge_chi=edge_chi, ped_width=ped_width, ne=False)
        pe, pe_popt, pe_perr, pe_chisqr = mtanh_fitting.best_osbourne(p_pe.X[idxs_pe,0][mask_pe], p_pe.y[idxs_pe][mask_pe], vals_unc=p_pe.err_y[idxs_pe][mask_pe], x_out=rhop_kp, maxfev=maxfev, reg=reg, edge_chi=edge_chi, ped_width=ped_width, ne=False)
        ...
        ne2, ne_popt2, ne_perr2, ne_chisqr2 = mtanh_fitting.best_osbourne(p_ne.X[idxs_ne,0][mask_ne], p_ne.y[idxs_ne][mask_ne], vals_unc=p_ne.err_y[idxs_ne][mask_ne], x_out=rhop_kp, maxfev=maxfev, reg=reg2, edge_chi=edge_chi, ped_width=ped_width, ne=True)
        Te2, Te_popt2, Te_perr2, Te_chisqr2 = mtanh_fitting.best_osbourne(p_Te.X[idxs_Te,0][mask_Te], p_Te.y[idxs_Te][mask_Te], vals_unc=p_Te.err_y[idxs_Te][mask_Te], x_out=rhop_kp, maxfev=maxfev, reg=reg2, edge_chi=edge_chi, ped_width=ped_width, ne=False)
        pe2, pe_popt2, pe_perr2, pe_chisqr2 = mtanh_fitting.best_osbourne(p_pe.X[idxs_pe,0][mask_pe], p_pe.y[idxs_pe][mask_pe], vals_unc=p_pe.err_y[idxs_pe][mask_pe], x_out=rhop_kp, maxfev=maxfev, reg=reg2, edge_chi=edge_chi, ped_width=ped_width, ne=False)
    ...
    grad_ne = get_fit_gradient(ne, ne_popt, rhop_kp, 'osborne', geqdsk, grad_type='analytic', out='R', reg=reg, plot=False)
    grad_Te = get_fit_gradient(Te, Te_popt, rhop_kp, 'osborne', geqdsk, grad_type='analytic', out='R', reg=reg, plot=False)
    grad_pe = get_fit_gradient(pe, pe_popt, rhop_kp, 'osborne', geqdsk, grad_type='analytic', out='R', reg=reg, plot=False)
    ...
    if frac_err: # this just takes a fraction of the profile as the errorbar
        ...
        f_ne.perturb_mc_err(p_ne, num_mc, maxfev, ped_width, reg, geqdsk, ne=True, verbose=False, plot=False)
        f_Te.perturb_mc_err(p_Te, num_mc, maxfev, ped_width, reg, geqdsk, ne=False, verbose=False, plot=False)
        f_pe.perturb_mc_err(p_pe, num_mc, maxfev, ped_width, reg, geqdsk, ne=False, verbose=False, plot=False)
            

        # now refit the mean and save coefficients of mean
    
        ne_mean_fit, ne_mean_popt, ne_mean_perr, ne_mean_chisqr = mtanh_fitting.best_osbourne(rhop_kp, f_ne.fit_mean, vals_unc=f_ne.fit_std, x_out=f_ne.x, maxfev=maxfev, reg=reg, edge_chi=edge_chi, ped_width=ped_width, ne=True)
        Te_mean_fit, Te_mean_popt, Te_mean_perr, Te_mean_chisqr = mtanh_fitting.best_osbourne(rhop_kp, f_Te.fit_mean, vals_unc=f_Te.fit_std, x_out=f_Te.x, maxfev=maxfev, reg=reg, edge_chi=edge_chi, ped_width=ped_width, ne=False)
        pe_mean_fit, pe_mean_popt, pe_mean_perr, pe_mean_chisqr = mtanh_fitting.best_osbourne(rhop_kp, f_pe.fit_mean, vals_unc=f_pe.fit_std, x_out=f_pe.x, maxfev=maxfev, reg=reg, edge_chi=edge_chi, ped_width=ped_width, ne=False)
...
def fetch_edge_probes(shot, time, Te_sep_eV, geqdsk=None, rhop_min=0.995, rhop_max=1.05, probes=['A'], max_ne_cm3=5e13, shift_probes=True):
    ...
    if 'A' in probes:
      try:
          ...
          rhop_asp, rhop_unc_asp, t_range, ne_prof_asp, ne_unc_prof_asp, Te_prof_asp, Te_unc_prof_asp, ax = out_asp
...
class kinetic_profile:
    ...
    def perturb_mc_err(self, raw, num_mc, maxfev, ped_width, reg, geqdsk, ne=True, verbose=True, plot=False):
        ...
        for ii in range(max_iter):
            ...
            try:
                _out = mtanh_fitting.best_osbourne(raw.X[:,0], new_pts, vals_unc=raw.err_y, x_out=self.x, maxfev=maxfev, reg=reg, ped_width=ped_width, ne=ne)
...
def load_fmp_neTe(shot, loc='outer', get_max=False, plot=False):
    ...
    for ii in probe_ind_list:
        node_ne = OMFITmdsValue(server='CMOD',shot=shot,treename='EDGE', TDI=f'\EDGE::top.probes.{probe_type}.{probe_loc}_0{ii}.p0.ne_Slow')
        node_Te = OMFITmdsValue(server='CMOD',shot=shot,treename='EDGE', TDI=f'\EDGE::top.probes.{probe_type}.{probe_loc}_0{ii}.p0.Te_Slow')
        node_Js = OMFITmdsValue(server='CMOD',shot=shot,treename='EDGE', TDI=f'\EDGE::top.probes.{probe_type}.{probe_loc}_0{ii}.p0.Js_Slow')

        # need to interpolate this onto ne/Te timebase
        if node_ne.data() is None:
            continue
        
        _node_rho = OMFITmdsValue(server='CMOD',shot=shot,treename='EDGE', TDI=f'\EDGE::top.probes.{probe_type}.{probe_loc}_0{ii}.p0.rho')
        rho_data = interp1d(_node_rho.dim_of(0), _node_rho.data(), bounds_error=False)(node_ne.dim_of(0))
    ...
    if get_max:
        ...
        return time, ne_fmp_max, rho_ne_max, Te_fmp_max, rho_Te_max, Js_fmp_max, rho_Js_max
    ...
    return time, ne_fmp_interp, rho_fmp_interp, Te_fmp_interp, rho_fmp_interp, Js_fmp_interp, rho_fmp_interp
...
def Lya_to_pflux(emiss_prof, ne, Te, coord, ni=None, rates_source='adas', coord_system='slab'):
    ...
    nn, _ = aurora.Lya_to_neut_dens(emiss_prof, ne, Te, plot=False, rates_source=rates_source)
    ...
    if np.sum(ion_rate_nonan) == 0:
        ...
        if first_trailing_zero < len(flux): flux[last_nonzero+2:] = np.nan # +2 because you want one more and then all on should be nan
