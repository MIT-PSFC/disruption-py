...
TOP_LEVEL_FOLDERS = [r"/fusion/projects/disruption_warning/disruption-warning-db-workflow/D3D/",
...
def search_folder(path):
    for file_name in os.listdir(path):
        if os.path.isdir(os.path.join(path,file_name)):
            ...
            ALL_SCRIPTS.append(
                (os.path.join(path, file_name), file_name[:-2]))
        else:
            pass
...
def add_script(path, parent=None):
    ...
    if method_name not in tree:
        tree.create_node(method_name + "\nInputs:" + str(input_args) +
                         "\nOutputs:" + str(output_args), method_name, parent=parent)
        dot.node(method_name,method_name + " | " + str(input_args) +" | " + str(output_args))
...
if '__main__' == __name__:
    ...
    search_script_for_paths(
        r'/fusion/projects/disruption_warning/disruption-warning-db-workflow/D3D/disruption_warning_database_d3d.m','root')
    tree.show()
    print(dot.source)
