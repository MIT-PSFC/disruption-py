...
def manually_check_profiles():
	...
	shot_list = [1160930034]
	time_slices = [0.36, 1.0, 1.32, 1.335]
	...
	print(df[['te_width_ece', 'te_core_vs_avg_ece', 'te_edge_vs_avg_ece', 'te0', 'te_width_err']])
	te_prof = pd.read_csv(f'{local_data_dir}te_prof.csv')
	print(te_prof)

	fig, axs = plt.subplots(5, 2, sharex='col')
	axs[0][0].plot(df['time'], df['te_width_ece'], marker='.')
	axs[0][0].set_ylabel("Te width [m]")
	axs[1][0].plot(df['time'], df['te_core_vs_avg_ece'], marker='.')
	axs[1][0].set_ylabel("Core PF")
	axs[2][0].plot(df['time'], df['te_edge_vs_avg_ece'], marker='.')
	axs[2][0].set_ylabel("Edge PF")
	axs[2][0].set_xlabel("time [s]")
	axs[3][0].plot(df['time'], df['te0'], marker='.')
