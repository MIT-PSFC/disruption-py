...
# TODO: Expand the batch run to full list of shots
# TODO: Make plots and write up changes
# TODO: Commit. Then clean and lint code
# TODO: Make pull request

# Shots to make plots:
# 1120824024 (example of radiative collapse with edge cooling)
# 1120808010 (Example of some issues: cutoff)
# 1151001017 Large sawteeth at 0.665, 0.668 s
# 1140416021- Shot with LH power and non-thermal emission after LH power turned of-- caught by sanity check
# 1140725001- Example of issues due to weird tansients in channels [0.4, 0.8, 1.5, 1.62, 1.63]
...
def manually_check_profiles():
	...
	shot_list = [1120824024]
	time_slices = [0.6, 0.614, 0.620, 0.624, 0.625]
	...
	print(df[['te_width_ece', 'te_core_vs_avg_ece', 'te_edge_vs_avg_ece', 'te0']])
	te_prof = pd.read_csv(f'{local_data_dir}te_prof.csv')
	print(te_prof)

	fig, axs = plt.subplots(5, 2, sharex='col', figsize=(14,6))
	axs[0][0].plot(df['time'], df['te_width_ece'], marker='.', ms=3)
	axs[0][0].set_ylabel("Te width [m]")
	axs[1][0].plot(df['time'], df['te_core_vs_avg_ece'], marker='.', ms=2)
	axs[1][0].set_ylabel("Core PF")
	axs[2][0].plot(df['time'], df['te_edge_vs_avg_ece'], marker='.', ms=2)
	axs[2][0].set_ylabel("Edge PF")
	axs[3][0].plot(df['time'], df['te0'], marker='.', ms=3)
	axs[3][0].set_ylabel("Te0 [keV]")
	axs[3][0].set_xlim(0., np.max(df['time']+0.05))
	axs[4][0].plot(df['time'], df['p_rad']/1e6, marker='.', ms=2)
	axs[4][0].set_ylabel('Prad [MW]')
	axs[-1][0].set_xlabel("Time [s]")

	# axs[4][0].plot(df['time'], df['te_width_err'])
	for i, t in enumerate(time_slices):
		print(i)
		prof = te_prof[te_prof['time']==t]
		if not prof.empty:
			axs[i][1].plot(prof['r'], prof['te'], marker='o', c='k', linestyle='none')
			rsample = np.arange(np.min(prof['r']), np.max(prof['r']), 0.001)
			amp = prof['te_fit_pa'].to_numpy()[0]
			mean = prof['te_fit_pmu'].to_numpy()[0]
			std = prof['te_fit_psigma'].to_numpy()[0]
			te_fit = gauss(rsample, amp, mean, std)
			axs[i][1].plot(rsample, te_fit, c='r', label='Fit')
...
def test_batch_run():
	...
	# rng.shuffle(shot_list)
	# shot_list = shot_list[:200]
	local_data_dir = '/home/henrycw/projects/disruption-py/drafts/local_data/'
	efit_name = 'efit21'
	...
	print(df[df['te_width_ece'] < 0.05].sort_values(by='te_width_ece', ascending=True))
	fig, axs = plt.subplots(1, 2, figsize=(14, 6))
	axs[0].hist(te_width_ece, bins=50)
	axs[0].set_yscale('log')
	axs[0].set_ylabel('Count')
	axs[0].set_xlabel('Te Width [m]')
	# 2D histogram with time
	mask = np.isfinite(df['time']) & np.isfinite(df['te_width_ece'])
	t = df[mask]['time'].to_numpy()
	w = df[mask]['te_width_ece'].to_numpy()
	h = axs[1].hist2d(t, w, bins=(100,100),
				   range = ((t.min(), t.max()), (w.min(), w.max())), 
				   cmap='viridis', norm=colors.LogNorm())
	fig.colorbar(h[3], ax=axs[1], label='Counts per bin')
	axs[1].set_xlabel('Time')
	axs[1].set_ylabel('Te width ECE')
	fig.suptitle('Distribution of Te Width from ECE \non C-Mod 2012-2016 EFIT21 Timebase')
	plt.show()
