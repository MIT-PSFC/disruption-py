...
if exist('radiated_fraction','var')==0;
  result = fetch(dbconn,['select radiated_fraction from ' ...
    'disruption_warning order by shot, time']);
  radiated_fraction = cell2mat(result);
end;
...
plot(-smear(time_until_disrupt(jj),.002), radiated_fraction(jj), '.b');
xlim([-0.1, 0]);
ylim([0, 1.5]);
set(gca, 'fontsize', 15);
...
print('pradfrac_vs_time_until_disrupt_d3d_iaea.png', '-dpng');
