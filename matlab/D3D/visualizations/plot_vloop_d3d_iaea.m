...
if exist('v_loop','var')==0;
  result = fetch(dbconn,['select v_loop from ' ...
    'disruption_warning order by shot, time']);
...
plot(-smear(time_until_disrupt(jj),.002),v_loop(jj).*ipsign(jj),'.b');
xlim([-.100, 0]);
ylim([-3,10]);
set(gca, 'fontsize', 15);
xlabel('Time until disrupt [s]', 'fontsize', 14);
ylabel('V_{loop} [volts]', 'fontsize', 14);
text(0.02, 0.90, 'DIII-D', 'fontsize', 16, 'fontweight', 'bold', ...
  'units', 'normalized', 'backgroundcolor', 'w', 'margin', 1);
hold on;
plot(xlim, [0,0], 'k', 'linewidth', 2);
hold off;
print('vloop_vs_time_until_disrupt_d3d_iaea.png', '-dpng');
