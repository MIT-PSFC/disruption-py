function [Mirnov_std_normalized, Mirnov_std]=get_Mirnov_std(shot,timebase,Mirnov_sensor)
...
Mirnov_std_normalized=NaN(nsensors, ntimes);
...
if (mod(status,2)==1)
   ...
   if (mod(statA,2)==1)
       ...
       btor=interp1(btor_time,btor,timebase, 'linear');
       btor=abs(btor);
       if (size(btor,1) > 1);
           btor=transpose(btor);
       end
...
for isensor = 1:nsensors;
  ...
  if (mod(statA,2)==1)
      Mirnov_std_normalized(isensor,:)=Mirnov_std(isensor,:)./btor;     
  end
