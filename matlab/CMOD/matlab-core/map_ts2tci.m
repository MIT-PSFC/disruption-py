...
if (mod(status,2)==0)
    fprintf(1, 'ERROR: No valid Ip available on shot %i\n', shot);
  return;
end
...
for i=1:nts 
  if flag
      ...
      [psimax,imax]=max(psitci(i,:));
  end
  for j=1:mts
    if flag
    if psits(i,j) >= psimin 
      a1=interp1(psitci(i,1:imin),ztci(1:imin),psits(i,j));
      a2=interp1(psitci(i,imin:end),ztci(imin:end),psits(i,j));
      ...
      nemapped_err(i,[j,j+mts])=nets_err(i,j);
         ...
         if psits(i,j) <= psimax 
      a1=interp1(psitci(i,1:imax),ztci(1:imax),psits(i,j));
      a2=interp1(psitci(i,imax:end),ztci(imax:end),psits(i,j));
      zmapped(i,[j,j+mts])=[a1,a2];
      nemapped(i,[j,j+mts])=nets(i,j);
      nemapped_err(i,[j,j+mts])=nets_err(i,j);
         end
