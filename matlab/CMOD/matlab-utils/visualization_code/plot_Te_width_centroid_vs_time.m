...
for it = 1:ntimes;
  ...
  centroid(it) = mean(Te_width_normalized(kk), 'omitnan');
end;
...
ylabel('<T_e width (normalized)>', 'fontsize', 14);
title('<T_e width (normalized)> vs time until disrupt', 'fontsize', 15);
...
ylabel('<T_e width (normalized)>', 'fontsize', 14);
title('<T_e width (normalized)>, non-disruptions', 'fontsize', 15);
