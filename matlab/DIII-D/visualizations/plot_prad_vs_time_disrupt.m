...
plot(-smear(time_until_disrupt(qq), 0.015), p_rad(qq)/1.e6,'.');
xlim([-0.25,0]);
ylim([0,15]);
set(gca, 'fontsize', 12)
xlabel('Time before disrupt [s]', 'FontSize', 14);
ylabel('P_{rad} [MW]', 'FontSize', 14);
title('P_{rad} vs time before disrupt (flattop ONLY)', 'FontSize', 16);
 
print -dpng d3d_prad_vs_time_disrupt.png;

figure('Name', ' ', 'NumberTitle','off','Menubar','none','Toolbar','none')
set(gcf, 'PaperPositionMode', 'auto');
plot(-smear(time_until_disrupt(qq), 0.015), radiated_fraction(qq),'.');
xlim([-0.25,0]);
ylim([0,2]);
set(gca, 'fontsize', 12)
xlabel('Time before disrupt [s]', 'FontSize', 14);
ylabel('P_{rad}/P_{input}', 'FontSize', 14);
title('P_{rad}/P_{input} vs time before disrupt (flattop ONLY)', ...
  'FontSize', 16);
 
print -dpng d3d_prad_frac_vs_time_disrupt.png;
