...
ExistingDataRequestType = Union[
    "ExistingDataRequest", str, pd.DataFrame, Dict[Tokamak, "ExistingDataRequestType"]
]
...
class ExistingDataRequestDict(ExistingDataRequest):
    def __init__(
        self, existing_data_request_dict: Dict[Tokamak, ExistingDataRequestType]
    ):
        resolved_existing_data_request_dict = {
            map_string_to_enum(tokamak, Tokamak): resolve_existing_data_request(
                individual_request
            )
    ...
    def _get_existing_data(self, params: ExistingDataRequestParams) -> pd.DataFrame:
        chosen_request = self.resolved_existing_data_request_dict.get(
            params.tokamak, None
        )
        if chosen_request is not None:
            ...
            params.logger.warning(
                f"No existing data request for tokamak {params.tokamak}"
            )
            return None
...
def resolve_existing_data_request(
    existing_data_request: ExistingDataRequestType,
) -> ExistingDataRequest:
    ...
    if isinstance(existing_data_request, str):
        existing_data_request_object = _existing_data_request_mappings.get(
