...
class EfitSetTimesRequest(SetTimesRequest):
    ...
    def cmod_times(self, params : SetTimesRequestParams):
        ...
        if efit_tree_name == 'analysis':
            try:
                return params.mds_conn.get(r"\analysis::efit_aeqdsk:time", tree_name="_efit_tree").astype('float64')
            except Exception as e:
                return params.mds_conn.get(r"\analysis::efit:results:a_eqdsk:time", tree_name="_efit_tree").astype('float64')
        else:
            return params.mds_conn.get(fr"\{efit_tree_name}::top.results.a_eqdsk:time", tree_name="_efit_tree").astype('float64')
...
class SignalSetTimesRequest(SetTimesRequest):
    ...
    def _get_times(self, params : SetTimesRequestParams) -> np.ndarray:
        try:
            ...
            return signal_time.astype('float64')
        except Exception as e:
