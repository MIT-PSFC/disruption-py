...
class EfitSetTimesRequest(SetTimesRequest):
    ...
    def cmod_times(self, params : SetTimesRequestParams):
        ...
        if efit_tree_name == 'analysis':
            try:
                return params.mds_conn.get(r"\analysis::efit_aeqdsk:time", tree_name="_efit_tree").astype('float64', copy=False)
            except Exception as e:
                return params.mds_conn.get(r"\analysis::efit:results:a_eqdsk:time", tree_name="_efit_tree").astype('float64', copy=False)
        else:
            return params.mds_conn.get(fr"\{efit_tree_name}::top.results.a_eqdsk:time", tree_name="_efit_tree").astype('float64', copy=False)
