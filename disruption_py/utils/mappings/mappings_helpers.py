...
def map_string_attributes_to_enum(obj, enum_translations : Dict):
    for field_name, enum_class in enum_translations.items():
        if hasattr(obj, field_name) and not isinstance(getattr(obj, field_name), enum_class):
            try:
...
def map_string_to_enum(value, enum_class):
    if isinstance(value, enum_class):
        ...
        try: 
            ...
            raise ValueError(f"Cannot convert value '{enum_value}' to enum for '{enum_class}'")
    return enum_value