...
class MethodOptimizer:
    ...
    def __init__(
        ...
        visited_method_names = {parameter_method.name for parameter_method in parameter_cached_method_props}
        parameter_cached_method_props_stack = parameter_cached_method_props.copy()
        while len(parameter_cached_method_props_stack) != 0:
            cached_method_props : CachedMethodProps = parameter_cached_method_props_stack.pop()
            ...
            if cached_method_props.name in pre_cached_method_names:
                continue
            graph.setdefault(cached_method_props.name, MethodOptimizer.MethodDependecy())
            # contained cached methods is a list of names of used cached methods
            for contained_cached_method_name in cached_method_props.get_param_value("contained_cached_methods", []):
                if contained_cached_method_name not in self._cached_method_props_by_name:
                    continue
                if contained_cached_method_name not in visited_method_names:
                    visited_method_names.add(contained_cached_method_name)
                    parameter_cached_method_props_stack.append(self._cached_method_props_by_name[contained_cached_method_name])
                # get method dependencies for contained cached method, creating one if it does not already exist
                contained_cached_method_dependency = graph.setdefault(contained_cached_method_name, MethodOptimizer.MethodDependecy())
                # add in contained cached method that cached method is dependent on it
                contained_cached_method_dependency.dependencies.add(cached_method_props.name)
                # add that cached method is dependent on contained cached method 
                # (contained cached method must complete before it can run)
                graph[cached_method_props.name].dependent_on.add(contained_cached_method_name)
    ...
    def next_method(self) -> CachedMethodProps:
        ...
        for cached_method_name in methods_to_consider:
            ...
            remaining_open_tree_count = sum(self._tree_remaining_count[tree_name] for tree_name in used_trees) - len(used_trees)
            close_estimate = len([tree_name for tree_name in used_trees
                                            if self._tree_remaining_count[tree_name] == 1 and tree_name in open_tree_names])
            open_estiamte = len([tree_name for tree_name in used_trees
                                            if self._tree_remaining_count[tree_name] != 1 and tree_name not in open_tree_names])
