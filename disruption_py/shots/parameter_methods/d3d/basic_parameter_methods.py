...
class BasicD3DRequests(ShotDataRequest):
    ...
    def _get_time_until_disrupt(params: ShotDataRequestParams):
        if params.shot_props.disrupted:
            return pd.DataFrame(
                {
                    "time_until_disrupt": params.shot_props.disruption_time
                    - params.shot_props.times
                }
            )
        return pd.DataFrame(
            {"time_until_disrupt": np.full(params.shot_props.times.size, np.nan)}
        )
    ...
    def get_H_parameters(params: ShotDataRequestParams):
        try:
            ...
            params.logger.info(
                f"[Shot {params.shot_props.shot_id}]: Failed to get H98 signal. Returning NaNs."
            )
            params.logger.debug(
    ...
    def get_power_parameters(params: ShotDataRequestParams):
        ...
        a_struct = get_bolo(
            params.shot_props.shot_id, bol_channels, bol_prm, bol_signals, bol_times
        )
        ier = 0
        for j in range(48):
    ...
    def get_ohmic_parameters(params: ShotDataRequestParams):
        ...
        try:
            v_loop, t_v_loop = params.mds_conn.get_data_with_dims(
                f'ptdata("vloopb", {params.shot_props.shot_id})', tree_name="d3d"
            )
        ...
        try:
            ip, t_ip = params.mds_conn.get_data_with_dims(
                f"ptdata('ip', {params.shot_props.shot_id})", tree_name="d3d"
            )
        ...
        dipdt_smoothed = interp1(
            t_ip, dipdt_smoothed, params.shot_props.times, "linear"
        )
    ...
    def get_density_parameters(params: ShotDataRequestParams):
        ...
        try:
            ...
            ip, t_ip = params.mds_conn.get_data_with_dims(
                f"ptdata('ip', {params.shot_props.shot_id})", tree_name="_efit_tree"
            )  # [A], [ms]
            t_ip = t_ip / 1.0e3  # [ms] -> [s]
    ...
    def get_rt_density_parameters(params: ShotDataRequestParams):
        ...
        try:
            ...
            ne_rt, t_ne_rt = params.mds_conn.get_data_with_dims(
                f"ptdata('dssdenest', {params.shot_props.shot_id})"
            )
            ...
            try:
                ip_rt, t_ip_rt = params.mds_conn.get_data_with_dims(
                    f"ptdata('ipsip', {params.shot_props.shot_id})"
                )  # [MA], [ms]
                t_ip_rt = t_ip_rt / 1.0e3  # [ms] to [s]
                # TODO: look at units of ip_rt (not SA)
            except Exception as e:
                ip_rt, t_ip_rt = params.mds_conn.get_data_with_dims(
                    f"ptdata('ipspr15v', {params.shot_props.shot_id})"
                )  # [MA], [ms]
                t_ip_rt = t_ip_rt / 1.0e3  # [ms] to [s]
    ...
    def get_ip_parameters(params: ShotDataRequestParams):
        ...
        try:
            ip_prog, t_ip_prog = params.mds_conn.get_data_with_dims(
                f"ptdata('iptipp', {params.shot_props.shot_id})", tree_name="d3d"
            )  # [A], [ms]
            t_ip_prog = t_ip_prog / 1.0e3  # [ms] -> [s]
            polarity = np.unique(
                params.mds_conn.get_data(
                    f"ptdata('iptdirect', {params.shot_props.shot_id})", tree_name="d3d"
                )
            )
            if len(polarity) > 1:
                params.logger.info(
        ...
        try:
            ipimode, t_ipimode = params.mds_conn.get_data_with_dims(
                f"ptdata('ipimode', {params.shot_props.shot_id})", tree_name="d3d"
            )
        ...
        try:
            epsoff, t_epsoff = params.mds_conn.get_data_with_dims(
                f"ptdata('epsoff', {params.shot_props.shot_id})", tree_name="d3d"
            )
    ...
    def get_rt_ip_parameters(params: ShotDataRequestParams):
        ...
        try:
            ip_rt, t_ip_rt = params.mds_conn.get_data_with_dims(
                f"ptdata('ipsip', {params.shot_props.shot_id})", tree_name="d3d"
            )  # [MA], [ms]
        ...
        try:
            ip_prog_rt, t_ip_prog_rt = params.mds_conn.get_data_with_dims(
                f"ptdata('ipsiptargt', {params.shot_props.shot_id})", tree_name="d3d"
            )  # [MA], [ms]
            ...
            ip_prog_rt = interp1(
                t_ip_prog_rt, ip_prog_rt, params.shot_props.times, "linear"
            )
        ...
        try:
            ip_error_rt, t_ip_error_rt = params.mds_conn.get_data_with_dims(
                f"ptdata('ipeecoil', {params.shot_props.shot_id})", tree_name="d3d"
            )  # [MA], [ms]
    ...
    def get_z_parameters(params: ShotDataRequestParams):
        ...
        try:
            z_cur, t_z_cur = params.mds_conn.get_data_with_dims(
                f"ptdata('vpszp', {params.shot_props.shot_id})", tree_name="d3d"
            )
    ...
    def get_n1_bradial_parameters(params: ShotDataRequestParams):
        ...
        b_tor, t_b_tor = params.mds_conn.get_data_with_dims(
            f"ptdata('bt', {params.shot_props.shot_id})", tree_name="d3d"
        )
    ...
    def get_peaking_factors(params: ShotDataRequestParams):
        ...
        try:
            ...
            rad_cva, t_rad_cva = params.mds_conn.get_data_with_dims(
                f"ptdata('dpsradcva', {params.shot_props.shot_id})", tree_name="d3d"
            )
        ...
        try:
            ...
            params.logger.info(
                f"[Shot {params.shot_props.shot_id}]:Failed to get TS data"
            )
            params.logger.debug(
                f"[Shot {params.shot_props.shot_id}]:{traceback.format_exc()}"
            )
    ...
    def _get_p_rad(params: ShotDataRequestParams, fan="custom"):
        ...
        for i in range(48):
            ...
            bol_times.append(bol_time)
        a_struct = get_bolo(
            params.shot_props.shot_id, bol_channels, bol_prm, bol_signals, bol_times[0]
        )
        b_struct = power(a_struct)
