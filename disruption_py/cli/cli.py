...
def setup_lazy_load_script(parser : argparse.ArgumentParser, module_name : str, entry_point_function_name="main", get_parser_function_name="get_parser"):
...
def main():
    ...
    setup_parser = main_subparsers.add_parser(
        "setup", 
        help="Setup additional dependencies for DisruptionPy"
    )
    ...
    run_parser = main_subparsers.add_parser(
        "run", 
        help="Run scripts using DisruptionPy"
    )
