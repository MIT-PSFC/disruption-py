...
class EASTPhysicsMethods:
    ...
    def get_efit_gaps(params: PhysicsMethodParams):
        ...
        xfirstwall = params.mds_conn.get_data(r"\top.results.geqdsk:xlim", tree_name="_efit_tree")
        yfirstwall = params.mds_conn.get_data(r"\top.results.geqdsk:ylim", tree_name="_efit_tree")
        ...
        xupperwall_mat = np.reshape(np.kron(xupperwall, seed), (len(seed, ), -1, len(efittime)))
        xlowerwall = xfirstwall_mat[index_lowerwall, :]
        xlowerwall_mat = np.reshape(np.kron(xlowerwall, seed), (len(seed, ), -1, len(efittime)))
        
        yupperwall = yfirstwall_mat[index_upperwall, :]
        yupperwall_mat = np.reshape(np.kron(yupperwall, seed), (len(seed, ), -1, len(efittime)))
        ylowerwall = yfirstwall_mat[index_lowerwall, :]
        ylowerwall_mat = np.reshape(np.kron(ylowerwall, seed), (len(seed, ), -1, len(efittime)))
        
        xupperplasma_mat = np.reshape(np.tile(xcoords, (len(xupperwall), 1)) (-1, len(xupperwall), len(efittime)))
        yupperplasma_mat = np.reshape(np.tile(ycoords, (len(yupperwall), 1)) (-1, len(yupperwall), len(efittime)))
        xlowerplasma_mat = np.reshape(np.tile(xcoords, (len(xlowerwall), 1)) (-1, len(xlowerwall), len(efittime)))
        ylowerplasma_mat = np.reshape(np.tile(ycoords, (len(ylowerwall), 1)) (-1, len(ylowerwall), len(efittime)))
