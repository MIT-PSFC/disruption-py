...
class EastPhysicsMethods:
    ...
    @staticmethod
    @physics_method(columns=["shot_domain"], tokamak=Tokamak.EAST)
    def get_shot_domain(params: PhysicsMethodParams):
        ...
        shot_domain = np.full(len(dipprog_dt), np.nan)
        # Get flattop domain indices
        (indices_flattop,) = np.where(np.abs(dipprog_dt) <= threshold_dipprog_dt)
