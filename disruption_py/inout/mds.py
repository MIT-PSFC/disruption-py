...
class MDSConnection:
    ...
    def open_tree(self, tree_name: str):
        ...
        if self.last_open_tree != tree_name:
            logger.trace(
                shot_log_msg("Opening tree: {tree_name}"),
                shot=self.shot_id,
                tree_name=tree_name,
            )
    ...
    def get_data(
        ...
        logger.trace(shot_log_msg("Getting data: {path}"), shot=self.shot_id, path=path)
    ...
    def get_data_with_dims(
        ...
        logger.trace(
            shot_log_msg("Getting data and dims: {path}"), shot=self.shot_id, path=path
        )
    ...
    def get_dims(
        ...
        logger.trace(shot_log_msg("Getting dims: {path}"), shot=self.shot_id, path=path)
