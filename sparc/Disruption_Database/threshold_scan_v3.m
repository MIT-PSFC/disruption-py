function threshold_scan_v3(shotlist, lower_threshold, upper_threshold, t_d, T_low, T_high, parameter)
...
for i = 1:length(lower_threshold)
    ...
    % Now check to see if any PD shots that are past the threshold are also
    % disruption shots in D_data. If so, we need to remove them.
    
    shots_PD_past_threshold = unique(PD_data(is_past_threshold,1));
    D_temp = D_data;
    bool4 = ismember(D_temp(:,1), shots_PD_past_threshold);
    D_temp(bool4,:) = [];
    
    % Find D_data where time_until_disrupt is between T_high + 0.005 s and
    % T_low - 0.005s (typically between 20.5 and 9.5ms)

    bool3 = and(D_temp(:,3) < (T_high+0.0005), D_temp(:,3) > (T_low - 0.0005));
    D_temp = D_temp(bool3,:);
...
plot(upper_threshold, missed, '--k', 'linewidth', 2)
plot(upper_threshold, false_pos, 'b', 'linewidth', 2)
plot(upper_threshold, early, 'r', 'linewidth', 2)

ylim([0,length(unique(D_data(:,1)))])

% legend('Correctly ID Disruption', 'Missed/Late Disruption', 'False Positive', 'Early Warning', ...
%     'location', 'southoutside')
l =legend('Correctly ID Disruption', 'Missed/Late Warning', 'False Positive', 'Early Warning', ...
    'location', 'best');
set(l, 'fontsize', 14)

xlabel(['Threshold value on ' strrep(parameter, '_', ' ')], 'fontsize', 18);
%ylabel('Percentage');
ylabel('Number of shots', 'fontsize', 18);
title([num2str((T_high + 0.0005)*1e3), 'ms > time until disrupt > ', num2str((T_low - 0.0005)*1e3), 'ms'], ...
    'fontsize', 18);
set(gca, 'fontsize', 16)
