function [n_equal_1_amplitude, n_equal_1_phase] = ...
  get_n_equal_1_amplitude_highpass_v2(shot, timebase);

% In version 2, I employ Ted's high pass filter hp.m only on the intervals
% provided in timebase. This is because this filter is noncausal, but now
% it will be only noncausal within the time interval provided and not the
% whole shot.
%
...
%
% Updated by A Tinguely 2016-04-28 to use a high-pass filter to get rid of
% the drift in the integrated Bp signals. 
%
% Inputs:
%   shot = shot number
...
% Next, for each sensor, we will call in the data and use a high pass
% filter to remove the low frequency signal (which ultimately leads to
% drifts in the data which are different for each sensor). We define a
% cutoff frequency for the hp filter.
...
% The next step is to pass our bp_dot signal through a high pass filter. We
% will use hp.m, which involves a FFT and Inverse FFT, which are
% *non-causal*. Therefore, we will perform the high pass filter in
% intervals of the data with the end time of each interval corresponding to
% a point in the provided timebase. 

  index_start = 1;
  bp_dot_filt = NaN(size(bp_dot));
  
  for j = 1:length(timebase)
      
     [~,index_end] = min(abs(time - timebase(j)));
     
     bp_dot_filt(index_start:index_end) = ...
         real(hp(bp_dot(index_start:index_end), ...
         time(index_start:index_end), cutoff_freq));
     
     index_start = index_end + 1;
     
  end

% Finally, we have to finish by using the high pass filter on the rest of
% the data, from the last time in timebase to the end.
  
  bp_dot_filt(index_start:end) = ...
        real(hp(bp_dot(index_start:end), ...
        time(index_start:end), cutoff_freq));

% if (shot > 1160101000 && isensor == 4);  % correct polarity for 2016 data
%   bp_dot = -bp_dot;                      % for BP_GH_TOP (the relocated coil)
% end;

  Bp = cumtrapz(time, bp_dot_filt);  % Integrate bp_dot [T/s] to get Bp [T]
