function [amp_array] = get_n1_amplitude(shot, timebase, num_times)
...
% toroidal magnetic field, so the resulting n=1 amplitude ratio unitless.
%
...
%
% Outputs:
%   amp_array = array of n=1 mode amplitudes in B_poloidal = (dB_p,n=1)/B_T
%  
...
%because hiding things in your code is fun
...
for j = 1:5 % for each toroidal location
    ...
    bp1 = cumtrapz(t,bp_dot); % cumulative numerical integration (trapezoidal) [T]
    
    bp{j} = interp1(t,bp1,time); % only take the times we want in timebase to reduce computation time 
...
amp_array = amplitudes./B_T; % normalize B_p,n=1 amplitudes to B_T
