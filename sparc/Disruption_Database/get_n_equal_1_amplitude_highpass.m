...
% Now we will employ a window-based Finite Impulse Response (FIR) high-pass
% filter for each of the bp_dot signals. We use FIR because given an
% impulse, our system should return to zero after a finite amount of time.
% For a FIR filter of order N, the filter lasts exactly N+1 samples before
% it settles to zero. The default window type is Hamming. We also need to
% choose our cutoff frequency.

%n_filter = 50; % order of FIR filter, 34 was used as an MATLAB example
cutoff_freq = 100; % Hz
...
% When employing the high pass filter, we need to input our cutoff
% frequency normalized to the Nyquist frequency, which is half of the
% sampling rate of our signal. Nominally, this is about 5 MHz, but it is
% best to calculate the average from time above.

%    avg_sampling_rate = 1/mean(diff(time)); % Hz
%    Nyquist_freq = 0.5*avg_sampling_rate; % Hz

%    Wn = cutoff_freq/Nyquist_freq; % 0 < Wn < 1
%    Wn = 0.5;

% Using the MATLAB notation for fir1()
%    b_filter = fir1(n_filter, Wn, 'high');
%    bp_dot_filt = filter(b_filter,1,bp_dot); % NOTE: filter() is causal

    bp_dot_filt = real(hp(bp_dot,time,cutoff_freq));
