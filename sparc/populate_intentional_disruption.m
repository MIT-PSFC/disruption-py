function populate_intentional_disruption(shotlist, varargin);
...
% 86-91 and lines 422 to 432) were added to populate the 
...
mgi_disrupted_runs = int32([1120911,1120802,1120801,1120622, ...
    1120604,1120202]);
...
% Get 'intentional disruption' column
    run = int32(shot-mod(shot,1000))/1000; % get run number
    
    if (ismember(shot,disruption_shotlist)); % check if shot disrupted
        if (ismember(run,mgi_disrupted_runs)); % check if shot was MGI
            intentional_disrupt = ones(ntimes_for_db,1);
        else;
            intentional_disrupt = zeros(ntimes_for_db,1);
        end;
    else;
        intentional_disrupt = nan(ntimes_for_db,1);
    end;
