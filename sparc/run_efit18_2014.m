% Get list of all disruption shots and times in the C-Mod 2014 campaign

db = set_database('logbook');
result = fetch(db, ['select shot, t_disrupt from disruptions where ' ...
  'shot between 1140101000 and 1141231999 order by shot']);
close(db);
...
% All plasma shots in 2014 have the standard EFIT data at 20 ms intervals
...
% times.  (This was not done for the original C-Mod dataset, which consisted
% of all shots in 2015, but was done for data from additional years.)
