% Get list of all disruption shots and times in the C-Mod 2005 campaign
...
% All plasma shots in 2005 have the standard EFIT data at 20 ms intervals
...
for ishot = 1:nshots;
  ...
  if (shot == 1050728016 || shot == 1051110010);  % For some reason, EFIT
    continue;                                     % goes into an infinite loop
  end;                                            % on these two shots
...
% consisted of all shots in 2015, but was done for the 2016 data.)
...
% status = unix(['/usr/local/cmod/codes/efit/bin/efitdd '
  status = unix(['/home/wolfe/cvswork/efit/efitbuild64/efitdd ' ...
