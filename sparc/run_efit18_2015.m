% Get list of all disruption shots and times in the C-Mod 2015 campaign

db = set_database('logbook');
result = fetch(db, ['select shot, t_disrupt from disruptions where ' ...
  'shot between 1150101000 and 1160101000 order by shot']);
close(db);
...
% All plasma shots in 2015 have the standard EFIT data at 20 ms intervals
...
% Get the list of standard EFIT times in the C-Mod ANALYSIS tree
  ...
  ntimes_slow = length(efit_standard_times);
