...
indx_good_ip_error_fraction  = find(~isnan(ip_error_fraction));
...
indx_good_radiated_fraction  = find(~isnan(radiated_fraction));
indx_good_v_loop             = find(~isnan(v_loop));
indx_good_temp             = find(~isnan(Te_HWHM));
...
indx_good = intersect( indx_good, indx_good_ip_error_fraction);
...
indx_good = intersect( indx_good, indx_good_radiated_fraction);
indx_good = intersect( indx_good, indx_good_v_loop);
indx_good = intersect( indx_good, indx_good_temp);

bins = [-0.250 : 0.002 : 0.002] - 0.001;

histogram( -time_until_disrupt(indx_good), bins, 'FaceColor', 'g');
xlim([-0.1, 0.0]);
ylim([0,200]);
set(gca, 'fontsize', 12);
xlabel('Time before disrupt [s]', 'fontsize', 14);
%title({'Histograms of # of discharges containing ', ...
 %      'no NaNs (cyan) and at least one NaN (magenta)', ...
...
indx_bad_ip_error_fraction   = intersect(ii, find(isnan(ip_error_fraction)));
...
indx_bad_radiated_fraction   = intersect(ii, find(isnan(radiated_fraction)));
indx_bad_v_loop              = intersect(ii, find(isnan(v_loop)));
indx_bad_temp              = intersect(ii, find(isnan(Te_HWHM)));
...
indx_bad = union( indx_bad , indx_bad_ip_error_fraction);
...
indx_bad = union( indx_bad , indx_bad_radiated_fraction);
indx_bad = union( indx_bad , indx_bad_v_loop);
indx_bad = union( indx_bad , indx_bad_temp);


hold on;
histogram( -time_until_disrupt(indx_bad ), bins, 'FaceColor', 'o');
hold off;
