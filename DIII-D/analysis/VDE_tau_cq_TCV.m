...
df = csvread('TCV-VDE-table.csv');

VDE_shotlist = int32(unique(df(1:end,1)));
...
for ishot = 1:nshots;
  ...
  fprintf(1, 'Opening shot %i (%3i/%3i, %5.1f%%)\n', shot, ishot, nshots, ...
    100*ishot/nshots);
  ii = find(df(1:end,1) == shot);
  ip = df(ii,4);
  iptime = df(ii,2);

  zc = df(ii,5);

% The plasma current on VDE's often starts to fall
% about 10 ms before the current quench (CQ), so I will determine the
% pre-VDE current by taking a 4 ms average at 20 ms before the CQ.

  tdis = max(iptime);

  [~, indx1] = min(abs(iptime - (tdis - 10e-3 - 2e-3)));
  [~, indx2] = min(abs(iptime - (tdis - 10e-3 + 2e-3)));
...
% Find the extrema of Zcentroid during the current quench
  ...
  indices = find(iptime >= tdis-0.005 & iptime <= tdis+0.002);
...
save('TCV_taucq_80_20.mat', 'VDE_shotlist', 'taucq_80_20', 'z_extrema');
[~,~] = unix('chmod a=r TCV_taucq_80_20.mat');

%{
load('TCV_taucq_80_20.mat');
