...
plot(-time_until_disrupt(indices_flattop), ...         
  v_loop_efit(indices_flattop).*ipsign(indices_flattop),'.');
xlim([-1,0]);
ylim([0,9]);
xlabel('Time before disrupt [s]', 'FontSize', 14);
ylabel('EFIT Loop voltage (processed for sign of Ip) [V]', 'FontSize', 14);
title('EFIT Loop voltage vs time before disrupt f(lattop ONLY)', ...
  'FontSize', 16);
 
set(gcf, 'PaperPositionMode', 'auto');
print -dpng vloop_efit_plot_1.png;
...
ylabel('EFIT Loop voltage (processed for sign of Ip) [V]', 'FontSize', 14);
title('EFIT Loop voltage vs time before disrupt (flattop ONLY)', ...
  'FontSize', 16);
 
set(gcf, 'PaperPositionMode', 'auto');
print -dpng vloop_efit_plot_2.png;
...
plot(time(indices_nodisrupt_flattop), ...         
  v_loop_efit(indices_nodisrupt_flattop).* ...
              ipsign(indices_nodisrupt_flattop),'.');
xlim([0,2]);
ylim([0,9]);
xlabel('Time [s]', 'FontSize', 14);
ylabel('EFIT Loop voltage (processed for sign of Ip) [V]', 'FontSize', 14);
title('EFIT Loop voltage for non-disruptive shots (flattop ONLY)', ...
  'FontSize', 16);
 
set(gcf, 'PaperPositionMode', 'auto');
print -dpng vloop_efit_plot_3.png;
