function [d_data, nd_data, d_shots, nd_shots] = plot_disruption_data(shotlist, varargin)
...
parameters = lower(strtrim(varargin)); % parameters to be fetched
...
for i = 7:size(d_data,2)
   ...
   subplot(3,1,1)
   xlabel('time until disrupt (ms)','fontsize', 18);
   ylabel([num2str(n_d), ' disruption shots'],'fontsize', 18);
   title(strrep(parameters(i-6), '_', ' '), 'fontsize', 24);
   ax = gca;
   set(ax, 'fontsize', 16);
   ...
   subplot(3,1,3)
   xlabel('time (s)');
   ylabel([num2str(n_nd), ' non-disruptions'], 'fontsize', 18);
   ax = gca;
   set(ax, 'fontsize', 16);
   hold off;
     
   saveas(f,['/home/tinguely/Disruptions/Code/Disruption_Database/presentation_160413_figures/', parameters{i-6}, '.fig'])
