function disruption_warning_database_d3d_zhu(shotlist)
...
%  2020/03/11 - CR;  Added kappa_area
%  2020/05/05 - CR;  Cinco de Mayo changes to p_ech, p_input, and
%                    radiated_fraction. After discovering a better
%		     (calibrated) pointname for p_ech, need to repopulate
%		     SQL db to fix signals.
...
%  2021/02/01 - CR;  Populate db with H98.
%
...
% zcur and the programmed zcur (z_error)
  ...
  [z_error, z_error_norm, z_prog, zcur, zcur_norm] = ...
    get_Z_error_d3d(shot, times_for_db);
    ...
    field_counter = field_counter + 1;
    fields(field_counter) = {'zcur_normalized'};
    values(:,field_counter) = num2cell(zcur_norm);
 

    field_counter = field_counter + 1;
    fields(field_counter) = {'z_prog'};
    values(:,field_counter) = num2cell(z_prog);
    ...
    field_counter = field_counter + 1;
    fields(field_counter) = {'z_error_normalized'};
    values(:,field_counter) = num2cell(z_error_norm);


% Get rms n1 component from Mirnov coils

  [n1rms, n1rms_normalized] = get_n1rms_d3d(shot, times_for_db);

    field_counter = field_counter + 1;
    fields(field_counter) = {'n1rms_normalized'};
    values(:,field_counter) = num2cell(n1rms_normalized);
...
% Old version of peaking factors for electron density and temperature
  [Te_PF_CVA, ne_PF_CVA, ~] = ...
...
% Populate _RT peaking factors for electron density, temperature, radiation profiles
  [Te_PF, ne_PF, Rad_CVA, Rad_XDIV] = ...
    get_peaking_factors_d3d(shot, times_for_db);

    field_counter = field_counter + 1;
    fields(field_counter) = {'Te_peaking_CVA_RT'};
    values(:,field_counter) = num2cell(Te_PF);
  
    field_counter = field_counter + 1;
    fields(field_counter) = {'ne_peaking_CVA_RT'};
    values(:,field_counter) = num2cell(ne_PF);

    field_counter = field_counter + 1;
    fields(field_counter) = {'Prad_peaking_CVA_RT'};
    values(:,field_counter) = num2cell(Rad_CVA);
  
    field_counter = field_counter + 1;
    fields(field_counter) = {'Prad_peaking_XDIV_RT'};
    values(:,field_counter) = num2cell(Rad_XDIV);


  [delta, squareness, aminor] = get_shape_parameters(shot, times_for_db); 

    field_counter = field_counter + 1;
    fields(field_counter) = {'delta'};
    values(:,field_counter) = num2cell(delta);
  
    field_counter = field_counter + 1;
    fields(field_counter) = {'squareness'};
    values(:,field_counter) = num2cell(squareness);

    field_counter = field_counter + 1;
    fields(field_counter) = {'aminor'};
    values(:,field_counter) = num2cell(aminor);



  [H98] = get_H98_d3d(shot,times_for_db);
    field_counter = field_counter + 1;
    fields(field_counter) = {'H98'};
    values(:,field_counter) = num2cell(H98);
%}
  
  [H_alpha] = get_H_alpha_d3d(shot,times_for_db);
    field_counter = field_counter + 1;
    fields(field_counter) = {'H_alpha'};
    values(:,field_counter) = num2cell(H_alpha);
